...
import { type ScamType, SCAM_CONTRACTS, getAbiByType } from "../lib/contract";
...
interface WalletPanelProps {
  scamType?: ScamType;
  onError?: (type: "blacklist" | null) => void;
}

export function WalletPanel({ scamType = "honeypot", onError }: WalletPanelProps) {
  ...
  const [errorType, setErrorType] = useState<"blacklist" | "unknown" | null>(null);
  ...
  const config = SCAM_CONTRACTS[scamType];
  const contractAddress = config.contractAddress as `0x${string}`;
  const abi = getAbiByType(scamType);
  ...
  // Whitelist query (common to all types)
  ...
  const receiptQuery = useWaitForTransactionReceipt({
    hash: actionTx,
    query: { enabled: Boolean(actionTx) }
  });

  // Reset state when scam type changes
  useEffect(() => {
    setError(null);
    setErrorType(null);
  ...
  // Keep sell target in sync with owner
  useEffect(() => {
    if (ownerQuery.data) {
  ...
  // Refresh balance on tx confirmation
  useEffect(() => {
    if (receiptQuery.isSuccess) {
      balanceQuery.refetch();
      whitelistQuery.refetch();
    }
  }, [receiptQuery.isSuccess, balanceQuery, whitelistQuery]);
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      // Detect error type based on scam mechanism
      if (msg.includes("blacklisted")) {
        setError(t(`scamTypes.${scamType}.errorHint`) as string);
        setErrorType("blacklist");
        onError?.("blacklist");
      } else if (msg.includes("Trading is disabled")) {
        setError(t(`scamTypes.${scamType}.errorHint`) as string);
        setErrorType("blacklist");
        onError?.("blacklist");
      } else if (msg.includes("max transaction")) {
        setError(t(`scamTypes.${scamType}.errorHint`) as string);
        setErrorType("blacklist");
        onError?.("blacklist");
        ...
        setError((error?.shortMessage as string) || (t("wallet.errors.unknown") as string));
  ...
  return (
    <div className="relative flex flex-col gap-4 rounded-[2rem] bg-white p-4 shadow-2xl ring-1 ring-black/5 md:p-6">
      ...
      <div className="relative flex flex-col gap-1">
        ...
        <div className="rounded-2xl bg-slate-100 p-4 transition hover:bg-slate-50 border border-transparent hover:border-slate-200">
          <div className="flex justify-between mb-1">
            ...
            {mode === "sell" && <span className="text-xs text-slate-500">{config.symbol}: {balance}</span>}
          </div>
          <div className="flex items-center gap-3">
            <input
              type="number"
              value={mode === "buy" ? "0" : amount}
        ...
        <div className="rounded-2xl bg-slate-100 p-4 transition hover:bg-slate-50 border border-transparent hover:border-slate-200">
          <div className="flex justify-between mb-1">
            ...
            {mode === "buy" && <span className="text-xs text-slate-500">{config.symbol}: {balance}</span>}
          </div>
          <div className="flex items-center gap-3">
            <input
              type="number"
              value={mode === "buy" ? amount : "0"}
      ...
      <div className="grid grid-cols-2 gap-2 mt-2">
        <div className={`rounded-lg p-2 text-center text-xs font-bold ${whitelistQuery.data ? "bg-green-100 text-green-700 ring-1 ring-green-200" : "bg-slate-100 text-slate-400"}`}>
          {whitelistQuery.data ? (t("wallet.status.whitelistYes") as string) : (t("wallet.status.whitelistNo") as string)}
        </div>
        <div className={`rounded-lg p-2 text-center text-xs font-bold bg-orange-100 text-orange-700 ring-1 ring-orange-200`}>
          {config.icon} {t(`scamTypes.${scamType}.tag`) as string}
        </div>
      </div>
